...
In questa pagina verrà sviluppato un esempio completo, per estrarre le informazioni sul PNRR legate al Comune di Messina, a partire dalle fonti dati descritte in questa guida: [OpenCUP](../fonti/opencup/index.qmd), [ANAC](../fonti/anac/index.qmd) e [SCP](../fonti/scp/index.qmd).
...
Come formato di salvataggio, si sceglie `CSV (delimitato da virgola) (*.csv)`. In output il file [`opencup-focus-083048.csv`](../data/messina/opencup-focus-083048.csv).<br>
...
Il [dataset CUP](../fonti/anac/index.qmd#il-dataset-cup) pubblicato dall'Autorità Nazionale Anticorruzione è composto da **più di 3 milioni di record**, quindi un foglio elettronico è pressoché inutilizzabile per estrarre tutti i codici `CIG` corrispondenti ai codici `CUP` estratti sopra per Messina.<br>
...
::: {.callout}
## Filtrare i 3 milioni di record di ANAC

Si può usare il comodo ed efficiente [`qsv`](https://github.com/jqnatividad/qsv), in accoppiata con [Miller](https://miller.readthedocs.io/en/latest/), a partire dal file [`opencup-focus-083048-cup.csv`](../data/messina/opencup-focus-083048-cup.csv) con i soli codici CUP del Comune di Messina:

```{.bash}
qsv searchset -d ";" -i -s CUP opencup-focus-083048-cup.csv cup_csv.csv >anac-cup-cig-083048.csv
```

Con `-d ";"` si imposta il separatore di campo che in ANAC è `;`, con `-i -s CUP opencup-focus-083048-cup.csv` si cercano nel file ANAC nella colonna `CUP` tutti i record contenenti i codici CUP estratti per Messina.
:::
...
Ancora una volta, si fa notare che questa è un'operazione semplice, ma che richiedere alcune conoscenze di programmazione. Qui sotto, al click, un esempio di codice.

::: {.callout-note collapse="true"}
## Interrogare le API di ANAC ed estrarre i dati di insieme

È uno [script `bash`](../script/messina/messina.sh) basato su [Miller](https://miller.readthedocs.io/en/latest/) e su [flatterer](https://flatterer.opendata.coop/).
...
Per estrarre i dati relativi a Messina e al PNRR dalla [Banca Dati SCP](../fonti/scp/index.qmd), si possono usare ancora una volta i **codici `CIG`** presenti nel file [anac-cup-cig-083048-cig.csv](../data/messina/anac-cup-cig-083048-cig.csv) estratto sopra.
...
!if [ ! -f ../data/messina/scp-atti.csv ]; then\
  qsv searchset -i -s cig ../data/messina/anac-cup-cig-083048-cig.csv ../data/scp/v_od_atti.csv >../data/messina/scp-atti-083048.csv;\
fi

!if [ ! -f ../data/messina/scp-esiti.csv ]; then\
  qsv searchset -i -s cig ../data/messina/anac-cup-cig-083048-cig.csv ../data/scp/v_od_atti.csv >../data/messina/scp-esiti-083048.csv;\
fi

!if [ ! -f ../data/messina/scp-bandi.csv ]; then\
  qsv searchset -i -s cig ../data/messina/anac-cup-cig-083048-cig.csv ../data/scp/v_od_atti.csv >../data/messina/scp-bandi-083048.csv;\
fi
```
...
# esiti
qsv searchset -i -s cig anac-cup-cig-083048-cig.csv v_od_atti.csv >scp-esiti-083048.csv
# bandi
qsv searchset -i -s cig anac-cup-cig-083048-cig.csv v_od_atti.csv >scp-bandi-083048.csv
```
...
- [`scp-esiti-083048.csv`](../data/messina/scp-esiti-083048.csv), contiene i dati relativi agli esiti;
